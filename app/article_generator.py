...
class ArticleGenerator:
    ...
    def _build_article_context(self, context: str, article_type: ArticleType) -> str:
        ...
        match article_type:
            case ArticleType.OP_ED:
                final_context = context + self.context_manager.read_context_file("article_types", "op_ed.txt")
            case ArticleType.SUMMARY:
                final_context = context + self.context_manager.read_context_file("article_types", "summary.txt")
    ...
    def _build_tone_context(self, context: str, tone: Tone) -> str:
        ...
        match tone:
            case Tone.FRIENDLY:
                final_context = context + self.context_manager.read_context_file("tone", "friendly.txt")
            case Tone.PROFESSIONAL:
                final_context = context + self.context_manager.read_context_file("tone", "professional.txt")
            case Tone.CASUAL:
                final_context = context + self.context_manager.read_context_file("tone", "casual.txt")
            case Tone.FORMAL:
                final_context = context + self.context_manager.read_context_file("tone", "formal.txt")
        return final_context
